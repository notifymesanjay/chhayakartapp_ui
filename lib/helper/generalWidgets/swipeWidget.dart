...
class SwipeWidget extends StatefulWidget {
  ...
  const SwipeWidget({Key? key, required this.child, required this.height, required this.onSwipeCallback, required this.onSwipeValueCallback, required this.isActive, this.swipePercentageNeeded = 0.75})
...
class SwipeWidgetState extends State<SwipeWidget> with SingleTickerProviderStateMixin {
        ...
        onPanEnd: (details) async {
          if (widget.isActive) {
            ...
            if (delta > deltaNeededToBeSwiped) {
              _controller.animateTo(swipeValue, duration: const Duration(milliseconds: 300), curve: Curves.fastOutSlowIn);
              widget.onSwipeCallback();
            } else {
              _controller.animateTo(1.0, duration: const Duration(milliseconds: 300), curve: Curves.fastOutSlowIn);
